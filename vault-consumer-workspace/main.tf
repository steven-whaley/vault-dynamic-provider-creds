...
resource "aws_instance" "myapp" {
  ...
  monitoring                  = true
  subnet_id                   = data.tfe_outputs.vault_dynamic_provider_build.values.ec2_subnet_id[0]
...
resource "vault_pki_secret_backend_cert" "myapp" {
  backend     = data.tfe_outputs.vault_dynamic_provider_build.values.pki_int_path
  name        = data.tfe_outputs.vault_dynamic_provider_build.values.pki_int_role
  ttl         = "72h"
